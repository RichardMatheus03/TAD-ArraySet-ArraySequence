...
void testInsert()
{
    ...
    for(int i = 1; i <= 10; i++)
    {
        ...
        string inserted = "teste" + to_string(i); //string inserted = str.append(to_string(i));
...
void testAll()
{
    ...
    //Aqui os comandos eram para inserir no início,
    //Mas nos testes as inserções são no fim.
    //insertBegin foi trocado por insertEnd
    
    sequence->insertEnd("l");  //sequence->insertBegin("l");
    sequence->insertEnd("m");  //sequence->insertBegin("m");
    sequence->insertEnd("n");  //sequence->insertBegin("n");
...
void testInsertEnd()
{
    ...
    sequence->insertEnd("a"); //sequence->insertBegin("a");
    assert( sequence->getQuantity() == 1 );
    assert( sequence->get(1) == "a" );
    
    sequence->insertEnd("b"); //sequence->insertBegin("b");
    assert( sequence->getQuantity() == 2 );
    assert( sequence->get(1) == "a" );
    assert( sequence->get(2) == "b" );
    
    sequence->insertEnd("c"); //sequence->insertBegin("c");
    assert( sequence->getQuantity() == 3 );
    assert( sequence->get(1) == "a" );
    assert( sequence->get(2) == "b" );
    assert( sequence->get(3) == "c" );
    
    sequence->insertEnd("x"); //sequence->insertBegin("x");
    assert( sequence->getQuantity() == 4 );
    assert( sequence->get(1) == "a" );
    assert( sequence->get(2) == "b" );
    assert( sequence->get(3) == "c" );
...
void testRemoveEnd()
{
    ...
    //Aqui o comando era para inserir no início,
    //mas no teste a inserção é no fim.
    //insertBegin foi trocado por insertEnd
