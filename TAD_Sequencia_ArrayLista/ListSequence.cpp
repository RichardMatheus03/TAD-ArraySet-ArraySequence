...
std::string Sequence::get(int i)
{
    ...
    //if (curr == NULL) return ""; // nova linha : posição inexistente
...
{
    //Caso a inserção seja no início, chama insertBegin
    if(i <= 1 || first == NULL){
        insertBegin(element);
        return;
    }

    //Caso seja no final, chama insertEnd
    if(i>quantity){
        insertEnd(element);
        return;
    }

    //caso inserir em qualquer outra posição:
    ...
    Node<std::string>* node = new Node<std::string>(element, curr->getNext());
    curr->setNext(node);
    quantity++;
}
...
std::string Sequence::remove(int i)
{
    ...
    //Se a remoção for no início, chama removeBegin
    if(i <= 1){
        return removeBegin();
    }

    //Se a remoção for no final, chama removeEnd
    if(i >= quantity){
        return removeEnd();
    }

    //Caso a remoção seja em qualquer outra posição:
